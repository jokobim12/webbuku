        ...
        <div class="mt-16 flex justify-center gap-2 flex-wrap pagination-container">
            <!-- Prev Button -->
            <?php if ($page > 1): ?>
                <a href="<?php echo $base_url . $connector . 'page=' . ($page - 1); ?>" class="pagination-link w-10 h-10 flex items-center justify-center rounded-full bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 transition-colors">←</a>
            <?php endif; ?>
            ...
            if ($start_page > 1) {
                echo '<a href="'.$base_url . $connector . 'page=1" class="pagination-link w-10 h-10 flex items-center justify-center rounded-full bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 transition-colors">1</a>';
                if ($start_page > 2) echo '<span class="flex items-end px-2 text-gray-400">...</span>';
            }

            for ($i = $start_page; $i <= $end_page; $i++): 
                $activeClass = ($i == $page) ? 'bg-emerald-600 text-white font-bold shadow-lg border-emerald-600' : 'bg-white text-gray-600 hover:bg-gray-50 border-gray-200';
            ?>
            ...
            if ($end_page < $total_pages) {
                ...
                echo '<a href="'.$base_url . $connector . 'page='.$total_pages.'" class="pagination-link w-10 h-10 flex items-center justify-center rounded-full bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 transition-colors">'.$total_pages.'</a>';
            }
            ?>
